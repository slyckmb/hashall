...
| OP-29 | reliability | **UPDATED post-j35+manual** — 80→0 truly stopped (state=0). j30: removed 4 leeching; j33: repaired 8 stubs; j34: repointed 4 path-broken; j35: erased EGB Boot Camp + Diary; manual: Beetlejuice+UEFA repointed to cross-seed/FileList.io/ + hash-checked → state=1 complete=1. All 80 stopped torrents resolved. | 2026-06-26 |
...
| OP-46 | bug | RT repoint briefs used /stash/ host paths but rTorrent runs in Docker with /stash→/data volume mount; agent needed 2 repoint rounds (first with /stash/ paths failed hash-check; second with /data/ paths succeeded); briefs specifying RT `d.directory` target paths must use Docker-internal paths (/data/media/ not /stash/media/); INIT.md RT path mapping note: /stash/media/ on host = /data/media/ in RT container | 2026-06-25 |
...
| OP-48 | bug | `chatrap opscan` returned `open=0` on this repo because `_resolve_ops_file` in chatrap-lead.sh looks for `${repo_root}/OPS.md` first and found the blank chatrap template at root — the real OPS.md was at `docs/OPS.md`; fixed by migrating real content to root `OPS.md` and making `docs/OPS.md` a redirect stub; chatrap infra requires OPS.md at repo root, not under docs/ | 2026-06-26 |
