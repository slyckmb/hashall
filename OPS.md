...
| OP-69 | bug | Recurring RT/qB state regressions keep reappearing after manual repair: RT complete items fall back to `stoppedDL`/`stalledDL` with stale or missing `d.directory`, and qB passive mirror items can reappear as active upload (`stalledUP`/`uploading`) even after previous enforcement. Latest incident 2026-07-05: qB `f37b9983` His.Three.Daughters was `stalledUP` and was stopped to `stoppedUP`; RT `e679f7` and `e56e8c` were repaired again; RT `f9389496` and `8685d0e6` remained blocked by missing `.nfo` evidence. Root-cause and hardening needed: identify which workflow writes stale RT directories or resumes qB mirror items, add a post-repair RT/qB state watchdog, persist a baseline of allowed stoppedDL exceptions, alert on new RT `PD`/qB active upload deltas, and make mirror/passive enforcement idempotent after qB/RT restart and cache refresh. | j43 |
