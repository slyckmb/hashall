...
| OP-62 | bug | 440 qB torrents were at `missingFiles` state (0%) due to stale stash save_paths after rehome migration. Ad-hoc repair moved missingFiles to 0, but many items transitioned into stoppedDL/checking and require the hardened j54 stoppedDL pipeline. Fix current qB state with bucket→drain→apply→roundloop, rollback ledger, passive-state enforcement, and persistence verification. | j51 |
