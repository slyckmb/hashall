...
| OP-49 | bug | `chatrap job --name` requires `--bypass-mastery` on every post-clear dispatch — session mastery gate env vars (`gate_mastery_pass`, proof head/branch, `gate_mastery_chat_memory_used`) are not persisted across `/clear`; `chatrap session mastery-check` exits without writing them; workaround: always pass `--bypass-mastery` for Model A / CLI lead sessions; fix needed in chatrap: either persist gate vars in SESSION.md/env file at mastery-check time, or make `chatrap job` skip the gate when running as lead (not agent) | 2026-06-26 |
