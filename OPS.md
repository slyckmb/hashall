...
| OP-61 | perf | Pool orphan dedupe via cross-device content matching — 18,645 quick_hash matches (1,599 GB) across stash library, hotspare, and WD6TB; pool/media snapshot destroyed (809 GB freed); pilot SHA256-confirmed deletion (66 GB) executed; full SHA256 upgrade scan running on pool orphans (~5 hrs); after scan: delete all SHA256-confirmed orphan files, recover ~2.2 TB. Supersedes OP-54 migrate-all approach — dedupe is smarter (delete only what's confirmed safe elsewhere). | 2026-07-01 |
| OP-62 | bug | 440 qB torrents at `missingFiles` state (0%) — qB save_path points to stale stash paths (`/data/media/torrents/seeding/<tracker>/`) after rehome migration moved data stash→pool but didn't update qB. All have `~noHL` + `~rt-mirrored` tags. Files exist on pool at RT's save_path. 265 single-file (fast fix), 177 multi-file (need dir move). Fix: batch `set_location` from stash path → pool path + recheck. | 2026-07-01 |
