...
| OP-68 | reliability | RT `PD` holdouts/regression need scoped repair. 2026-07-05 scoped live fix: `e679f7` Black.Mirror.Bandersnatch.2018.2160p.NF.WEB-DL.DDP5.1.DV.H.265-WKS.mkv was repointed from missing `/data/.../DigitalCore (API)/...` path to `/pool/media/torrents/seeding/Black.Mirror.Bandersnatch.2018.2160p.NF.WEB-DL.DDP5.1.DV.H.265-WKS`, hash-checked to `complete=1`, and started; `e56e8c` Dexter.S02.720p.x265-ZMNT was repointed from missing `/data/.../TorrentLeech/...` path to `/pool/media/torrents/seeding/Dexter.S02.720p.x265-ZMNT/Dexter.S02.720p.x265-ZMNT`, hash-checked to `complete=1`, and started. Two additional RT `PD` holdouts remain blocked, not safe-started: `f9389496` Killers of the Flower Moon and `8685d0e6` The Matrix Reloaded have candidate `.mkv` payloads but required `.nfo` files from torrent metadata were not found, so they cannot be proven 100%. Continue with explicit hash-scoped evidence only; no broad stopped-torrent scans or speculative starts. | j39 |
| OP-69 | bug | Recurring RT/qB state regressions keep reappearing after manual repair, and the existing safety process was bypassable: RT complete items fall back to `stoppedDL`/`stalledDL` with stale or missing `d.directory`, and qB passive mirror items can reappear as active upload (`stalledUP`/`uploading`) even after previous enforcement. Latest incident 2026-07-05: qB `f37b9983` His.Three.Daughters was `stalledUP` and was stopped to `stoppedUP`; RT `e679f7` and `e56e8c` were repaired again; RT `f9389496` and `8685d0e6` remained blocked by missing `.nfo` evidence. Root-cause and hardening needed: identify which workflow writes stale RT directories or resumes qB mirror items; make full 4-Gate or surgical mini-gate mandatory for every live RT/qB mutation except qB stop-only containment; add a post-repair RT/qB state guard; persist a baseline of allowed stoppedDL exceptions; alert on new RT `PD`/qB active upload deltas; update prompts, mastery, briefs, docs, and runbooks so direct helper/API/XMLRPC mutation is no longer an informal path. | j57 |
