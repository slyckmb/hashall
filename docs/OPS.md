...
| OP-28 | reliability | 11 missingFiles appeared after qB restart — 4 pre-existing path-mismatch patterns exposed: (A) 4× double-nested release dir in cross-seed items; (B) 1× single-file torrent with internal dir but file placed loose at tracker root; (C) 2× FileList.io items pointed to FileList.io/ root instead of cross-seed/FileList.io/ (OP-24 items); (D) 2× main torrent pointed to non-existent pool path while content lives on stash; (E) 2× content displaced to hash-dir or dead staging path; all data confirmed on disk; repair = set_location + recheck per item | 2026-06-21 |
