...
| OP-41 | reliability | 3 course/BLURAY items were never seeding — started by leeching incident, partially downloaded, cannot be hardlink-repaired (no source): Coursera (2.2% downloaded), Domestika ×2 (0-1.6%), Priscilla.2023.MULTi.COMPLETE.BLURAY-MONUMENT (0.2%). Root cause: leeching incident called d.start; these cross-seed items had no matching source library content. Recommendation: remove all 3 torrents from RT (and delete partial download data); no content loss (never seeded). | 2026-06-25 |
