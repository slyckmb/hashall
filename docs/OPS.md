...
| OP-18 | reliability | EXPLORE: single-pass canonical path assignment for all ~4k RT items — codify one decision tree (category → seeding-root → path formula), walk every RT item, compute target, diff vs actual, migrate if needed, then sync qB; avoids piecemeal migrations with competing path specs; investigate feasibility before next migration job | 2026-06-17 |
