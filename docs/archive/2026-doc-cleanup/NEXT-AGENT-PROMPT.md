...
#4.5)
bin/db-refresh-step4_5-link-dedup.sh --apply

• Implementation is complete: new optional dedup/hardlink step is in place between step 3 and step 4, defaulting to dry-run. I’m sending the exact
  usage and recommended next command now.

• Inserted point: between step 4 and step 5.

  What I changed:

  - Added new script: bin/db-refresh-step3_5-link-dedup.sh (/home/michael/dev/work/hashall/.agent/worktrees/codex-hashall-20260225-152657/bin/db-
    refresh-step3_5-link-dedup.sh)
  - Updated step-4 footer note to reference step 4.5: bin/db-refresh-step3-sha256-backfill.sh (/home/michael/dev/work/hashall/.agent/worktrees/
    codex-hashall-20260225-152657/bin/db-refresh-step3-sha256-backfill.sh)

  How step 4.5 works:

  - Creates hashall link plan per device.
  - Shows plan summary.
  - Runs link execute --dry-run by default.
  - Only performs real hardlink creation with --apply.
  - Logs to ~/.logs/hashall/reports/db-refresh/step3_5-link-dedup-*.log.

  Recommended command now (safe preview):

  bin/db-refresh-step4_5-link-dedup.sh

  Apply command (actual hardlinking):

  bin/db-refresh-step4_5-link-dedup.sh --apply

  Optional controls:

  - --devices stash,data (default is stash,data,hotspare6tb)
  - --min-size 1048576 (default 1 MiB)
  - --limit 100 (max actions per device; default all)
