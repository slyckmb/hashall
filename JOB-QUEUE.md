...
| j51 | qb-stoppeddl-recovery | OP-62,OP-70,OP-78,OP-79,OP-80,OP-81,OP-82 | Active recovery in progress. t10+t11 live verified-retarget batches succeeded, later t12 work brought live qB to `stoppedDL=362`, `stoppedUP=4554`; approved t13-t20 live verified-retarget succeeded and moved qB to `stoppedDL=170`, `stoppedUP=4746`. Current near-complete class: 6 qB `stoppedDL` rows at `99.*%`; 2026-07-10 Plan A validation found 0 direct sibling-hardlink repair candidates. All 6 are now `needs-split-redownload`: rename invalid target file/tree out of the failed torrent path, redownload fresh bytes, then compare old vs new to prove duplicate vs real variant. OP-81 expands `payload sync --source rt --path-prefix` aliases before filtering; OP-82 skips whole-catalog orphan prune during hash-scoped sync. Both have regression coverage in `tests/test_cli_payload_sync.py`. Next: run gated Plan B on the six 99.* rows before treating them as waiting/replacement outcomes. |
