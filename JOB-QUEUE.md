...
| j51 | qb-stoppeddl-recovery | OP-62,OP-70,OP-78,OP-79 | Active recovery blocked before save-path repair. Verified 24-hash live attempt stopped/restarted qB but patched 0 because same-filesystem gate blocked all rows after service stop; fix dry-run/live gate mismatch before retry. |
...
| j51-t10 | blocked (same-filesystem gate) | Operator approved live apply for the 24 verified hashes. Live fastresume batch stopped qB, then blocked all 24 rows on same-filesystem checks (`storage_root_mismatch:other!=pool` or `device_mismatch:49!=45`), restarted qB, and was interrupted during an unnecessary wait on unchanged `stoppedDL`. Patched=0; no rollback ledger/report was written; qB is healthy; pause dry-run found no active mirror items; direct qB API check found all 24 still `stoppedDL`. Updated report copied to `.agent/reports/j51-ship-20260708/J51-T10-GUARDED-EXECUTION.md`. Next: fix dry-run/live same-filesystem mismatch before retry. |
