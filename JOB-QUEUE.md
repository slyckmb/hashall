...
| j51 | qb-stoppeddl-recovery | OP-62,OP-70,OP-78,OP-79 | Active recovery ready for an approval-gated verified-retarget pilot. OP-79 tool fix makes the conservative 24-hash dry-run block safely, and v0.2.14 adds an explicit verified fastresume retarget mode that dry-runs `same_filesystem_overridden=24` with all 24 still live `stoppedDL`. |
