...
| j51 | qb-stoppeddl-recovery | OP-62,OP-70,OP-78,OP-79,OP-80,OP-81,OP-82,OP-83,OP-84,OP-85 | Active recovery in progress. t10+t11 live verified-retarget batches succeeded, later t12 work brought live qB to `stoppedDL=362`, `stoppedUP=4554`; approved t13-t20 live verified-retarget succeeded and moved qB to `stoppedDL=170`, `stoppedUP=4746`. Current near-complete class: 6 qB `stoppedDL` rows at `99.*%`; 2026-07-10 Plan A validation found 0 direct sibling-hardlink repair candidates. All 6 are now `needs-split-redownload`: rename invalid target file/tree out of the failed torrent path, redownload fresh bytes, then compare old vs new to prove duplicate vs real variant. 2026-07-10 Plan B validation added `bin/rt-qb-variant-split-redownload.py`; repaired Spider-Man `5feb771c9b7f` and operator-approved Here `c5a827e36ebb` to qB `stoppedUP`. OP-83 caught and fixed the freeleech-proof/tracker mismatch gap; 2026-07-11 follow-up fixed DigitalCore API indexer-label matching. River Monsters `127c38342cfe` is active/waiting on TorrentDay proof; Transformers `96d896ca35f4` is active/waiting on DigitalCore proof with 0 seeders; both have 0 connected peers and 0 byte progress in the live sample. Dexter S02/S07 SpeedCD remain stopped pending strict SpeedCD proof or explicit operator approval. OP-84 is implemented: qB recheck monitors now treat moving `stoppedDL` progress/amount-left as active verification and only fail stable stoppedDL after baseline/grace. OP-85 is implemented: `client-drift verify-pieces` can now map failed pieces to media/sidecar file spans and exact payload/quarantine roots. OP-81 expands `payload sync --source rt --path-prefix` aliases before filtering; OP-82 skips whole-catalog orphan prune during hash-scoped sync. Next: use failed-piece mapping before any further Plan B start; parked active/no-peer items are not blockers; find strict proof or approval for the remaining SpeedCD rows, then continue one controlled Plan B start at a time. |
