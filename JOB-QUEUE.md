...
| j51 | qb-stoppeddl-recovery | OP-62,OP-70,OP-78,OP-79,OP-80,OP-81 | Active recovery in progress. t10+t11 live verified-retarget batches succeeded, later t12 work brought live qB to `stoppedDL=362`, `stoppedUP=4554`; approved t13-t20 live verified-retarget succeeded and moved qB to `stoppedDL=170`, `stoppedUP=4746`. Current near-complete class: 9 qB `stoppedDL` rows at `99.*%`, all classified as `tracker-healthy-waiting` rather than automatic replacement-needed. OP-81 working-tree fix now expands `payload sync --source rt --path-prefix` aliases before filtering, with RT regression coverage. Next: review/commit OP-81 and continue read-only verification expansion from t21 for remaining stoppedDL rows. |
...
- j39 (cross-seed-repair) after j42 — requires canonicalize drift items corrected (j46+j47+j48 done) and lane2 strategy settled; includes OP-68 RT `PD` holdouts/regression. 2026-07-05 surgical dry-runs keep `f9389496` and `8685d0e6` blocked because current RT directories lack required payload files. 2026-07-09 RCCA: the current 99% RT `PD` rows are stopped after failed completion hash checks and share hardlinked inodes with 100% siblings, so do not simply start them; split the incomplete hash to a unique payload tree before any wait-for-seeds start. 2026-07-10 exception/repair: `0b236c5155a4` E.T. seedpool was split/rehomed to `/pool/media/torrents/seeding/cross-seed/seedpool (API)/`, RT/qB/DB now canonical and complete; old source staged, not deleted. Report: `docs/RCCA-RT-PD-HOLDOUTS-20260709.md`.
