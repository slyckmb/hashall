...
| j51 | qb-stoppeddl-recovery | OP-62,OP-70,OP-78,OP-79 | Active recovery ready for approval-gated verified-retarget pilots. OP-79 v0.2.14 adds explicit verified fastresume retarget mode. Four batches are prepared: t10=24, t11=22, t12=24, t13=25; all 95 are still live `stoppedDL`. |
...
- j51 (qb-stoppeddl-recovery, OP-62/OP-70/OP-78/OP-79) active recovery ready for approval-gated verified-retarget pilots — OP-62 started as 440 missingFiles. Current 2026-07-08 refresh: qB `stoppedDL=441`, `stoppedUP=4475`, active upload/download=0; bucket `/tmp/qb-stoppeddl-bucket-live` has 441 hashes. DB refresh artifacts live under `.agent/reports/db-refresh-j51-20260707-045304/`. Correction after t07: generic catalog-drain policy was the wrong first lens; direct RT session mapping shows all 441 qB stoppedDL hashes are present in rTorrent and all 441 RT paths exist. Original t09 path-shape Class A evidence was not apply-compatible, so the first approved 25-hash batch was re-verified: 24 verified, 1 failed partial-match hash (`376c463cd58324`). Live apply for the 24 verified hashes stopped/restarted qB but patched 0 because same-filesystem gate blocked all rows after qB was stopped. OP-79 v0.2.13 made the conservative dry-run block before service stop (`blocked=24`, `same_filesystem_blocked=24`). OP-79 v0.2.14 added explicit `--allow-verified-fastresume-cross-filesystem`, limited to independently verified fastresume-only retargets. Prepared batches: t10 has 24 verified, dry-run `same_filesystem_overridden=24`; t11 has 22 verified from the next 25 Class A candidates, dry-run `same_filesystem_overridden=22`, with 3 partial-match exclusions; t12 has 25 verified but 1 root-policy rejection (`ef1071a1` Elemental, separate j52 anomaly), leaving 24 approval-ready with dry-run `same_filesystem_overridden=24`; t13 has 25 verified with dry-run `same_filesystem_overridden=25`. Live qB checks confirm all 95 prepared approval-ready hashes are still `stoppedDL`. Next: with explicit operator approval, run the 24-hash t10 pilot first, verify stoppedDL delta and persistence, then run t11/t12/t13 batches if the pilot passes.
...
| j51-t10 | approval-gated (verified fastresume retarget) | Operator approved live apply for the 24 verified hashes. Initial live fastresume batch stopped qB, then blocked all 24 rows on same-filesystem checks (`storage_root_mismatch:other!=pool` or `device_mismatch:49!=45`), restarted qB, and was interrupted during an unnecessary wait on unchanged `stoppedDL`. Patched=0; no rollback ledger/report was written; qB is healthy; direct qB API check found all 24 still `stoppedDL`. OP-79 fix implemented in two steps: v0.2.13 made conservative dry-run/live preflight block before service stop; v0.2.14 added explicit verified fastresume retarget override for independently verified ratio=1.0 rows. Current dry-runs: conservative `blocked=24`; verified-retarget `blocked=0`, `same_filesystem_overridden=24`; live-state check: all 24 still `stoppedDL`. Next: operator approval required for live verified-retarget pilot using `/tmp/qb-stoppeddl-bucket-live/reports/j51-t10-verified-first25-drain.json`. |
