...
| j51 | qb-stoppeddl-recovery | OP-62,OP-70,OP-78,OP-79,OP-80,OP-81,OP-82,OP-83,OP-84 | Active recovery in progress. t10+t11 live verified-retarget batches succeeded, later t12 work brought live qB to `stoppedDL=362`, `stoppedUP=4554`; approved t13-t20 live verified-retarget succeeded and moved qB to `stoppedDL=170`, `stoppedUP=4746`. Current near-complete class: 6 qB `stoppedDL` rows at `99.*%`; 2026-07-10 Plan A validation found 0 direct sibling-hardlink repair candidates. All 6 are now `needs-split-redownload`: rename invalid target file/tree out of the failed torrent path, redownload fresh bytes, then compare old vs new to prove duplicate vs real variant. 2026-07-10 Plan B validation added `bin/rt-qb-variant-split-redownload.py`; repaired Spider-Man `5feb771c9b7f` and operator-approved Here `c5a827e36ebb` to qB `stoppedUP`. OP-83 caught and fixed the freeleech-proof/tracker mismatch gap. Tracker-matched rescan found River Monsters has exact TorrentDay freeleech proof with 1 seeder and it is now active/waiting; Dexter S02/S07 SpeedCD are blocked by size mismatch/0 seeders; Transformers DigitalCore is proofed but 0 seeders. OP-84 is implemented: qB recheck monitors now treat moving `stoppedDL` progress/amount-left as active verification and only fail stable stoppedDL after baseline/grace. OP-81 expands `payload sync --source rt --path-prefix` aliases before filtering; OP-82 skips whole-catalog orphan prune during hash-scoped sync. Next: leave River Monsters active and recheck progress; do not start other Plan B rows without tracker-matched freeleech proof or explicit operator approval. |
