...
| j57 | rt-qb-state-guard | OP-69 | Merged to CR via direct implementation in `f06e944` with direct-CR workflow caveat. Guard tools/docs/prompts are usable now; root-cause follow-up remains open. |
| j59 | chatrap-standard-upgrade | OP-89 | Next. Bring hashall onto the latest chatrap infra/lead standard before more repair work: `.chatrap` runtime state, generated read-only control views, current dispatch flow, `.chatrap` logs/task-logs, gitignore/untracking cleanup, and lead status/prepare-clear validation. |
...
- j59 is the next job and should run before j51/j58/j52 because current chatrap guidance says this repo's tracking model is outdated: markdown control files are views, `.chatrap/state` is runtime authority, and lead source mutations must be dispatched rather than edited inline.
...
j50 → j57 → j59 → j51 → j58 → j52 → j55 → j42 → j39 → j43 → j56 → j45
...
- j59 (chatrap-standard-upgrade, OP-89) is now next because latest chatrap guidance shows hashall's session infra is behind standard: `.chatrap/state/queue.json` is empty, `chatrap lead status` has no dispatchable next job, legacy `state/*.json` and markdown control files are still committed, `.gitignore` is missing chatrap runtime patterns, and recent lead work was done inline instead of through `chatrap dispatch`. This job upgrades the repo/workflow before more operational repair.
...
- Current/next briefs:
  ...
  - `j59-t01` latest chatrap standard audit — written
  - `j59-t02` state authority migration plan/implementation — written
  - `j59-t03` gitignore and tracked-control cleanup — written
  - `j59-t04` lead dispatch workflow update — written
  - `j59-t05` validation and prepare-clear gate — written
  - `j59-t06` closeout docs and follow-up OP triage — written
...
## j59 — chatrap-standard-upgrade

**Slug:** chatrap-standard-upgrade
**OPs:** OP-89
**Goal:** Bring hashall's chatrap integration up to the latest standard before further live repair. The job must migrate authority away from tracked markdown/legacy state, initialize usable `.chatrap/state` queue/ops/jobs data without committing `.chatrap/`, update lead workflow so source mutations go through `chatrap dispatch`, and validate that `chatrap lead status`, prep/ship/wrap, and prepare-clear see a dispatchable next job.
**Scope:** Repo/tracking/tooling integration only. Do not mutate live RT/qB state.

### Tasks

| Task | Status | Goal |
|------|--------|------|
| j59-t01 | planned | Audit latest chatrap guidance vs hashall state. Produce a precise gap report covering `.chatrap/` runtime rules, generated MD views, legacy `state/*.json`, `.agent` vs `.chatrap` paths, missing gitignore patterns, lead dispatch bypasses, empty `.chatrap/state/queue.json`, and prepare-clear/lead-status failures. |
| j59-t02 | planned | Build the state-authority migration. Convert current OP/job/run-order data into `.chatrap/state/ops.json`, `.chatrap/state/queue.json`, and `.chatrap/state/jobs.json` using current chatrap schema; preserve hold-chatrap and j59 as next; render `OPS.md`/`JOB-QUEUE.md` only as read-only views if the current tooling supports it. |
| j59-t03 | planned | Apply infra file hygiene. Add missing chatrap gitignore patterns, stop tracking deprecated generated/control artifacts where latest chatrap requires it, and prove no `.chatrap/` runtime files are staged. If untracking a file is unsafe or unsupported by current hashall hooks, document the blocker and add a follow-up OP. |
| j59-t04 | planned | Update hashall lead/brief workflow guidance. Replace stale `opencode run`, `.agent/logs`, direct-edit, and old task-log paths with `chatrap dispatch`, Pi/direct dispatch tiers, `.chatrap/logs`/`.chatrap/task-logs`, and the "no inline source mutation" rule. Update reusable briefs/runbooks so future lead work is dispatched instead of edited inline. |
| j59-t05 | planned | Validate the upgraded workflow. Run `chatrap lead status`, state sync/validate commands, brief lint/checks, `chatrap prep`/dry readiness for the next job, and `chatrap session prepare-clear`. The expected result is a clean tree, nonempty next-job state, no tracked `.chatrap` files, and no `next=unknown` prepare-clear warning. |
| j59-t06 | planned | Closeout and handoff. Document final repo-specific chatrap operating rules, update queue notes, record any blockers as OPs, and prepare the next job after j59 without reverting the migration. |
...
**OPs:** OP-62, OP-70, OP-78, OP-79, OP-80, OP-81, OP-82, OP-83, OP-84, OP-85, OP-86, OP-87
...
| j51-t10 | done (live verified fastresume retarget) | Operator approved live apply for the 24 verified hashes. Initial live fastresume batch had patched 0 because same-filesystem gate blocked after qB stop; OP-79 v0.2.13/v0.2.14 fixed that with pre-stop gate and explicit verified-retarget override. Approved live rerun succeeded: report `/tmp/qb-stoppeddl-bucket-live/reports/j51-t10-verified-retarget-apply-live.json` has `planned=24`, `applied=24`, `ok=24`, `failed=0`, `fr_patched=24`, `rollback_ledger_written=24`; rollback ledger `/tmp/qb-stoppeddl-bucket-live/reports/j51-t10-verified-retarget-rollback-ledger.jsonl` has 24 entries; post-state `/tmp/qb-stoppeddl-bucket-live/reports/j51-t10-verified-24-live-state-after-retarget.json` confirms all 24 are `stoppedUP`; global qB count changed `stoppedDL=441→417`, `stoppedUP=4475→4499`; qB container healthy; pause mirror dry-run found no active mirror items. |
| j51-t11 | done (read-only verification expansion) | Verified the next 25 Class A RT-first candidates with offline libtorrent checks. Result: 22 exact verified, 3 partial-match exclusions (`e4446638c792`, `245f2bce6afa`, `e36553b12dc`). Native apply report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t11-verified-next25-drain.json`; hash file: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t11-verified-22-hashes.txt`; dry-run report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t11-verified-next25-apply-dryrun-verified-retarget.json` with `planned=22`, `blocked=0`, `same_filesystem_overridden=22`; live-state report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t11-verified-22-live-state-before-retarget.json` confirms all 22 still `stoppedDL`. |
| j51-t11-live | done (live verified fastresume retarget) | Operator approved via “Go next”. Live retarget succeeded: report `/tmp/qb-stoppeddl-bucket-live/reports/j51-t11-verified-retarget-apply-live.json` has `planned=22`, `applied=22`, `ok=22`, `failed=0`, `fr_patched=22`, `rollback_ledger_written=22`; rollback ledger `/tmp/qb-stoppeddl-bucket-live/reports/j51-t11-verified-retarget-rollback-ledger.jsonl` has 22 entries; post-state `/tmp/qb-stoppeddl-bucket-live/reports/j51-t11-verified-22-live-state-after-retarget.json` confirms all 22 are `stoppedUP`; global qB count changed `stoppedDL=417→395`, `stoppedUP=4499→4521`; qB container healthy; pause mirror dry-run found no active mirror items. |
| j51-t12 | done (read-only verification expansion) | Verified the next 25 Class A RT-first candidates with offline libtorrent checks. Result: 25 exact verified, but 1 root-policy rejection (`ef1071a139c5` Elemental resolved to forbidden `/data/media/torrents/seeding/movies`, keep with j52 anomaly work), leaving 24 approval-ready. Native apply report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t12-verified-next25-drain.json`; approval hash file: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t12-verified-24-approval-ready-hashes.txt`; dry-run report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t12-verified-next25-apply-dryrun-verified-retarget.json` with `planned=24`, `root_policy_rejected=1`, `blocked=0`, `same_filesystem_overridden=24`; live-state report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t12-verified-24-live-state-before-retarget.json` confirms all 24 approval-ready hashes still `stoppedDL`. |
| j51-t13 | done (read-only verification expansion) | Verified the next 25 Class A RT-first candidates with offline libtorrent checks. Result: 25 exact verified, 0 failed. Native apply report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t13-verified-next25-drain.json`; hash file: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t13-verified-25-hashes.txt`; dry-run report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t13-verified-next25-apply-dryrun-verified-retarget.json` with `planned=25`, `blocked=0`, `same_filesystem_overridden=25`; live-state report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t13-verified-25-live-state-before-retarget.json` confirms all 25 still `stoppedDL`. |
| j51-t13-filter | done (read-only current-state filter) | Filtered t13 against current qB state after later repairs. Result: 24 current `stoppedDL` hashes remain approval-ready; one prior t13 hash was already `stoppedUP`. Native apply report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t13-verified-24-current-stoppeddl-drain.json`; hash file: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t13-verified-24-current-stoppeddl-hashes.txt`; dry-run report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t13-verified-24-current-stoppeddl-apply-dryrun.json` with `planned=24`, `blocked=0`, `same_filesystem_overridden=24`. |
| j51-t14 | done (read-only verification expansion) | Verified the next candidate slice. Result: 18 approval-ready hashes; 7 excluded for now (6 missing verifier reports/no-candidate rows and one huge Nintendo set timed out at 19.6% after 900s). Native apply report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t14-verified-18-drain.json`; hash file: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t14-verified-18-hashes.txt`; dry-run report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t14-verified-18-apply-dryrun.json` with `planned=18`, `blocked=0`, `same_filesystem_overridden=18`. |
| j51-t15 | done (read-only verification expansion) | Verified the next current `stoppedDL` slice with direct libtorrent checks against RT paths. Result: 25 verified, 0 failed. Native apply report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t15-verified-25-drain.json`; hash file: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t15-verified-25-hashes.txt`; dry-run report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t15-verified-25-apply-dryrun.json` with `planned=25`, `blocked=0`, `same_filesystem_overridden=25`. |
| j51-t16 | done (read-only verification expansion) | Verified the next current `stoppedDL` slice after excluding already handled/prepared hashes and the 9 near-99 monitor rows. Result: 25 verified, 0 failed. Native apply report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t16-verified-25-drain.json`; hash file: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t16-verified-25-hashes.txt`; dry-run report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t16-verified-25-apply-dryrun.json` with `planned=25`, `blocked=0`, `same_filesystem_overridden=25`. |
| j51-t17 | done (read-only verification expansion) | Verified the next current `stoppedDL` slice after excluding already handled/prepared hashes and the 9 near-99 monitor rows. Result: 25 verified, 0 failed, including `e679f7d2501f` Bandersnatch. Native apply report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t17-verified-25-drain.json`; hash file: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t17-verified-25-hashes.txt`; dry-run report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t17-verified-25-apply-dryrun.json` with `planned=25`, `blocked=0`, `same_filesystem_overridden=25`. |
| j51-t18 | done (read-only verification expansion) | Verified the next current `stoppedDL` slice after excluding already handled/prepared hashes and the 9 near-99 monitor rows. Result: 25 verified, 0 failed, including two independent Goosebumps hashes. Native apply report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t18-verified-25-drain.json`; hash file: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t18-verified-25-hashes.txt`; dry-run report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t18-verified-25-apply-dryrun.json` with `planned=25`, `blocked=0`, `same_filesystem_overridden=25`. |
| j51-t19 | done (read-only verification expansion) | Verified the next current `stoppedDL` slice after excluding already handled/prepared hashes and the 9 near-99 monitor rows. Result: 25 verified, 0 failed, including duplicate-title Inside Out and Dead Shot hashes verified independently. Native apply report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t19-verified-25-drain.json`; hash file: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t19-verified-25-hashes.txt`; dry-run report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t19-verified-25-apply-dryrun.json` with `planned=25`, `blocked=0`, `same_filesystem_overridden=25`. |
| j51-t20 | done (read-only verification expansion) | Verified the next current `stoppedDL` slice after excluding already handled/prepared hashes and the 9 near-99 monitor rows. Result: 25 verified, 0 failed, including duplicate-title Megalopolis, Merry Ever After, and Peppermint hashes verified independently. Native apply report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t20-verified-25-drain.json`; hash file: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t20-verified-25-hashes.txt`; dry-run report: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t20-verified-25-apply-dryrun.json` with `planned=25`, `blocked=0`, `same_filesystem_overridden=25`. |
| j51-t13-t20-live | done (live verified fastresume retarget) | Operator approved live apply for the combined t13-t20 prepared batches, 192 hashes total. Combined dry-run report `/tmp/qb-stoppeddl-bucket-live/reports/j51-t13-t20-approved-192-apply-dryrun-prelive.json` had `planned=192`, `blocked=0`, `guard_daemon_detected=0`; live report `/tmp/qb-stoppeddl-bucket-live/reports/j51-t13-t20-approved-192-apply-live.json` has `planned=192`, `applied=192`, `ok=192`, `failed=0`, `fr_patched=192`, `rollback_ledger_written=192`; rollback ledger has 192 entries; post-state report `/tmp/qb-stoppeddl-bucket-live/reports/j51-t13-t20-approved-192-post-state.json` confirms all 192 are `stoppedUP`; global qB count changed `stoppedDL=362→170`, `stoppedUP=4554→4746`; qB container healthy; pause mirror dry-run found no active mirror items. |
| j51-t21 | done (99.* content-variant Plan A validation) | Documented `docs/RT-QB-99PCT-CONTENT-VARIANT-REPAIR.md` and extended `bin/torrent-sibling-hardlink-repair.py` to support multi-file `--source-dir` verification/hardlink plans. Validated all 6 current qB `99.* stoppedDL` rows with dry-runs: 0 direct sibling-hardlink repair candidates. Corrected next state: all 6 are `needs-split-redownload`, meaning Plan B must rename the invalid target file/tree away, let the failed torrent download fresh bytes, then compare old vs new to prove duplicate vs real variant. Report: `docs/J51-99PCT-CONTENT-VARIANT-VALIDATION-20260710.md`; dry-runs: `/tmp/hashall-j51-*-dryrun.json`; tests: `tests/test_torrent_sibling_hardlink_repair.py` 2 passed. |
| j51-t22 | in_progress (99.* Plan B split/redownload validation) | Added `bin/rt-qb-variant-split-redownload.py` and tests for hash-scoped Plan B split/quarantine/start. Dry-ran all 6 current 99.* candidates: all had shared inodes and planned split reports under `.agent/reports/j51-variant-split-validation-20260710/`. Limited pilot on `c5a827e36ebb` validated the paused-after-start symptom: before split RT was stopped with failed-completion message and target inode `dev=49 ino=4066 nlink=5`; after quarantine + `d.check_hash` + `d.start`, RT transitioned to active download on isolated inode `dev=49 ino=100406 nlink=1`, while the healthy sibling stayed on inode `4066`; guard check passed. Follow-up split/containment moved the remaining five candidates off shared inodes and stopped all six pending tracker/freeleech/operator decision; final dry inventory has `split_eligible_shared_count=0`; post-containment guard passed with 0 issues. Dexter S02 nested sibling `e56e8c574db0` was restored and rechecked complete after the live evidence exposed a whole-root multi-file hazard; tool v0.2.0 now blocks nested RT session dirs and requires operator approval/freeleech proof before start. Added `bin/prowlarr-freeleech-proof.py` to search Prowlarr read-only and produce JSON proof only from explicit freeleech fields/flags. Added `--start-existing-split` so an already contained row can be resumed only after proving its payload path exists, is isolated, has no nested RT session dir, and is not complete. `5feb771c9b7f` Spider-Man was selected as the first representative from explicit TorrentLeech freeleech proof with viable seeds; RT downloaded and rechecked to `complete=1 left=0`, qB rechecked from 99.9979% stoppedDL to `stoppedUP progress=1.0 left=0`, and old-vs-new comparison proved same-size different bytes at byte 57. Hash-scoped RT payload sync processed one complete payload and skipped orphan prune; fixed the `--upgrade-missing` empty-queue summary crash exposed by this path. Added required placement audit: canonical path shape is per torrent, but stash-vs-pool home is decided for the full same-inode payload group; any media-library member keeps the whole group on stash. Targeted Spider-Man placement audit found one same-inode member, no media-library member, and `group_home=pool_eligible` for the current new variant group. Proposed-member audit with Aither `5c86280a99d1` found media-library anchors and `stash_required`; offline piece verification of the new `5feb` file against Aither failed 1/1514 pieces, so Aither is a separate old variant and must not be hardlinked/repointed to the new variant. Follow-up source scan exposed broad-query false proof in `bin/prowlarr-freeleech-proof.py`; v0.2.0 now supports expected title/size filters. OP-83/tool v0.2.1 now blocks freeleech-proof starts unless proof indexer matches the actual RT torrent tracker host; 2026-07-11 patch also accepts mapped API labels like `DigitalCore (API)` when tracker hosts match. Tracker-matched scan found: River Monsters `127c38342cfe` exact TorrentDay freeleech proof with 1 seeder; Dexter S02/S07 SpeedCD rows blocked by missing strict proof; Transformers exact DigitalCore freeleech proof but 0 seeders; Here exact PrivateHD seeded hit but no freeleech proof. River Monsters is active (`state=1`) but waiting with 0 connected peers/0 byte progress. Operator approved Here `c5a827e36ebb`; RT downloaded and verified it complete, offline piece verification passed `6384/6384`, qB recheck moved it to `stoppedUP` 100%, and qB/RT hash-scoped payload syncs both recorded complete payloads. Transformers `96d896ca35f4` dry-run passed after the DigitalCore alias fix, then live start applied; RT is active (`state=1`) with `left_bytes=21082531706`, 0 connected peers, and 0 byte progress, so it is parked waiting rather than blocking. OP-84 is implemented in `_monitor_rt_qb_rechecks()`: moving `stoppedDL` progress/amount-left is treated as active verification, first stoppedDL sample establishes a baseline, and stable stoppedDL still fails after grace. OP-85 extends `client-drift verify-pieces` with failed-piece file-span mapping, JSON output, exact `--payload-root`/`--quarantine-root`, and Makefile vars `SHOW=1 MAP=1 JSON=1 PAYLOAD_ROOT=... QUARANTINE_ROOT=...`. Reports: `docs/J51-VARIANT-SPLIT-VALIDATION-20260710.md`, `.agent/reports/j51-tracker-matched-source-scan-20260711/96d896ca35f4-start-existing-live-digitalcore.json`, `.agent/reports/j51-tracker-matched-source-scan-20260711/live-sample-after-transformers-start.txt`; tests: Plan B focused suite passed; `tests/test_client_drift.py` 38 passed, 1 xfailed; `tests/test_torrent_verify.py` 5 passed. Pending: run failed-piece mapping before further Plan B starts; find strict SpeedCD proof or operator approval for Dexter S02/S07; parked active/no-peer items are not blockers. |
| j51-t23 | planned | Implement OP-86: replace the one-off qB partial fastresume `pieces` restore with guarded hash-scoped tooling, dry-run/live reports, rollback ledger, path-shape validation, and post-restart verification. |
| j51-t24 | planned | Implement OP-87: update `qb-stoppeddl-apply.py` so a moving qB recheck after live apply is reported as pending/needs-monitor instead of failed, with a focused monitor report path. |
...
Replanned 2026-07-14 (latest chatrap standard upgrade): added OP-89 and new next job j59 after reviewing latest chatrap guidance/infra. Root gap: hashall is still operating from committed markdown/legacy state and inline lead edits while latest chatrap requires `.chatrap/state` runtime authority, generated read-only MD views, current `.chatrap` pathing, and dispatched lead source mutations. Run order is now j50→j57→j59→j51→j58→j52→j55→j42→j39→j43→j56→j45. Briefs written for j59-t01 through j59-t06.
