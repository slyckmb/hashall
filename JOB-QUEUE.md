...
| j51 | qb-stoppeddl-recovery | OP-62,OP-70,OP-78,OP-79,OP-80,OP-81,OP-82 | Active recovery in progress. t10+t11 live verified-retarget batches succeeded, later t12 work brought live qB to `stoppedDL=362`, `stoppedUP=4554`; approved t13-t20 live verified-retarget succeeded and moved qB to `stoppedDL=170`, `stoppedUP=4746`. Current near-complete class: 6 qB `stoppedDL` rows at `99.*%`; 2026-07-10 Plan A validation found 0 direct sibling-hardlink repair candidates. All 6 are now `needs-split-redownload`: rename invalid target file/tree out of the failed torrent path, redownload fresh bytes, then compare old vs new to prove duplicate vs real variant. 2026-07-10 Plan B validation added `bin/rt-qb-variant-split-redownload.py`; dry-ran all 6 current candidates and live-piloted `c5a827e36ebb`, which moved from stopped after failed-completion hash to active download on an isolated `nlink=1` target inode while the healthy sibling inode stayed intact. OP-81 expands `payload sync --source rt --path-prefix` aliases before filtering; OP-82 skips whole-catalog orphan prune during hash-scoped sync. Both have regression coverage in `tests/test_cli_payload_sync.py`. Next: let/verify `c5a827e36ebb` complete, compare quarantined vs new bytes, then repeat Plan B for the remaining 99.* rows. |
