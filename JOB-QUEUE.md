...
| j51-t22 | in_progress (99.* Plan B split/redownload validation) | Added `bin/rt-qb-variant-split-redownload.py` and tests for hash-scoped Plan B split/quarantine/start. Dry-ran all 6 current 99.* candidates: all had shared inodes and planned split reports under `.agent/reports/j51-variant-split-validation-20260710/`. Limited pilot on `c5a827e36ebb` validated the paused-after-start symptom: before split RT was stopped with failed-completion message and target inode `dev=49 ino=4066 nlink=5`; after quarantine + `d.check_hash` + `d.start`, RT transitioned to active download on isolated inode `dev=49 ino=100406 nlink=1`, while the healthy sibling stayed on inode `4066`; guard check passed. Report: `docs/J51-VARIANT-SPLIT-VALIDATION-20260710.md`; tests: 51 focused tests passed. Pending: let `c5a827e36ebb` finish, compare quarantined vs new bytes, DB sync, then close or repeat. |
