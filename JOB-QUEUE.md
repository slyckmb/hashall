...
| j51 | qb-stoppeddl-recovery | OP-62,OP-70,OP-78 | Active recovery planning at approval gate. First approved 25-hash batch was reworked into a verified/apply-compatible 24-hash dry-run plan; live apply still requires fresh explicit approval. |
...
- j51 (qb-stoppeddl-recovery, OP-62/OP-70/OP-78) active recovery planning at approval gate — OP-62 started as 440 missingFiles. Current 2026-07-07 refresh: qB `stoppedDL=441`, `stoppedUP=4478`, `checking=0`, active upload/download=0; bucket `/tmp/qb-stoppeddl-bucket-live` has 441 hashes. DB refresh artifacts live under `.agent/reports/db-refresh-j51-20260707-045304/`. Correction after t07: generic catalog-drain policy was the wrong first lens; direct RT session mapping shows all 441 qB stoppedDL hashes are present in rTorrent and all 441 RT paths exist. Original t09 path-shape Class A evidence was not apply-compatible, so the first approved 25-hash batch was re-verified. Result: 24 verified hashes, 1 failed partial-match hash (`376c463cd58324`). Apply dry-run against `/tmp/qb-stoppeddl-bucket-live/reports/j51-t10-verified-first25-drain.json` planned 24 in `fastresume_batch_wait` mode with 0 root-policy rejects and 0 same-filesystem blocks. No qB mutation ran. Next: explicit operator approval for the 24-hash verified live apply.
...
- Current j51 briefs:
  ...
  - `j51-t10` live execution — approval required for verified 24-hash fastresume batch
...
| j51-t10 | approval required (verified dry-run ready) | Operator approved first 25 Class A hashes, but no live qB mutation ran. Offline verification completed for the first 25: 24 verified, 1 failed partial-match hash (`376c463cd58324cdb1b5645930227055b3053837`). Generated native/apply-compatible report `/tmp/qb-stoppeddl-bucket-live/reports/j51-t10-verified-first25-drain.json` and archived it at `.agent/reports/j51-ship-20260708/J51-T10-VERIFIED-FIRST25-DRAIN.json`. Apply dry-run planned 24 in `fastresume_batch_wait` mode with 0 root-policy rejects and 0 same-filesystem blocks. Updated report copied to `.agent/reports/j51-ship-20260708/J51-T10-GUARDED-EXECUTION.md`. Fresh explicit approval required before live `--apply`. |
