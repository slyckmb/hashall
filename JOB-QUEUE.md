...
| j51 | qb-stoppeddl-recovery | OP-62,OP-70,OP-78,OP-79,OP-80,OP-81,OP-82 | Active recovery in progress. t10+t11 live verified-retarget batches succeeded, later t12 work brought live qB to `stoppedDL=362`, `stoppedUP=4554`; approved t13-t20 live verified-retarget succeeded and moved qB to `stoppedDL=170`, `stoppedUP=4746`. Current near-complete class: 6 qB `stoppedDL` rows at `99.*%`; 2026-07-10 runbook validation found 0 verified sibling-hardlink repair candidates, 4 one-piece `source-not-compatible` rows, and 2 `waiting-for-seeds-or-replacement` missing/truncated sidecar-content rows. OP-81 expands `payload sync --source rt --path-prefix` aliases before filtering; OP-82 skips whole-catalog orphan prune during hash-scoped sync. Both have regression coverage in `tests/test_cli_payload_sync.py`. Next: continue non-99 stoppedDL verification expansion and keep the 6 99.* monitor/replacement rows explicit. |
...
- j39 (cross-seed-repair) after j42 — requires canonicalize drift items corrected (j46+j47+j48 done) and lane2 strategy settled; includes OP-68 RT `PD` holdouts/regression. 2026-07-05 surgical dry-runs keep `f9389496` and `8685d0e6` blocked because current RT directories lack required payload files. 2026-07-09 RCCA: the current 99% RT `PD` rows are stopped after failed completion hash checks and share hardlinked inodes with 100% siblings, so do not simply start them; split the incomplete hash to a unique payload tree before any wait-for-seeds start. 2026-07-10 exception/repair: E.T. sibling group resolved. `0b236c5155a4` seedpool, `1c6285d80aa3` Darkpeers, and `4b4a1747e01b` DigitalCore now point at per-tracker pool paths hardlinked to the verified seedpool inode; qB is `stoppedUP` 100%, RT is `complete=1 left=0`, and DB has all three complete payload rows on device 45. Report: `docs/RCCA-RT-PD-HOLDOUTS-20260709.md`.
...
| j51-t21 | done (99.* content-variant runbook + validation) | Documented `docs/RT-QB-99PCT-CONTENT-VARIANT-REPAIR.md` and extended `bin/torrent-sibling-hardlink-repair.py` to support multi-file `--source-dir` verification/hardlink plans. Validated all 6 current qB `99.* stoppedDL` rows with dry-runs: 0 verified sibling-hardlink repair candidates; 4 one-piece `source-not-compatible` rows (`245f2bce6afa`, `e36553b12dc1`, `c5a827e36ebb`, `5feb771c9b7f`); 2 `waiting-for-seeds-or-replacement` rows with missing/truncated sidecar content (`127c38342cfe`, `96d896ca35f4`). Report: `docs/J51-99PCT-CONTENT-VARIANT-VALIDATION-20260710.md`; dry-runs: `/tmp/hashall-j51-*-dryrun.json`; tests: `tests/test_torrent_sibling_hardlink_repair.py` 2 passed. |
