...
| j51 | qb-stoppeddl-recovery | OP-62,OP-70,OP-78,OP-79 | Active recovery in progress. t10+t11 live verified-retarget batches succeeded: qB `stoppedDL` dropped 441→395 and `stoppedUP` rose 4475→4521. Next approval-ready batches: t12=24, t13=25. |
...
- j51 (qb-stoppeddl-recovery, OP-62/OP-70/OP-78/OP-79) active recovery in progress — OP-62 started as 440 missingFiles. Current 2026-07-08 post-t11 refresh: qB `stoppedDL=395`, `stoppedUP=4521`, active upload/download=0; bucket `/tmp/qb-stoppeddl-bucket-live` has 441 original hashes. DB refresh artifacts live under `.agent/reports/db-refresh-j51-20260707-045304/`. Correction after t07: generic catalog-drain policy was the wrong first lens; direct RT session mapping shows all 441 qB stoppedDL hashes are present in rTorrent and all 441 RT paths exist. Original t09 path-shape Class A evidence was not apply-compatible, so the first approved 25-hash batch was re-verified: 24 verified, 1 failed partial-match hash (`376c463cd58324`). Initial live apply for the 24 verified hashes stopped/restarted qB but patched 0 because same-filesystem gate blocked all rows after qB was stopped. OP-79 v0.2.13 made the conservative dry-run block before service stop (`blocked=24`, `same_filesystem_blocked=24`). OP-79 v0.2.14 added explicit `--allow-verified-fastresume-cross-filesystem`, limited to independently verified fastresume-only retargets. Approved t10 live pilot succeeded: `applied=24`, `ok=24`; qB global count dropped 441→417. Approved t11 live batch also succeeded: `/tmp/qb-stoppeddl-bucket-live/reports/j51-t11-verified-retarget-apply-live.json` reports `applied=22`, `ok=22`, `failed=0`, `fr_patched=22`, `rollback_ledger_written=22`; exact-state report `/tmp/qb-stoppeddl-bucket-live/reports/j51-t11-verified-22-live-state-after-retarget.json` confirms all 22 are `stoppedUP`; qB global count dropped 417→395; qB healthy; pause mirror dry-run found no active mirror items. Next approval-ready batches: t12 has 24 approval-ready after excluding Elemental (`same_filesystem_overridden=24`), t13 has 25 verified (`same_filesystem_overridden=25`). Next: with explicit operator approval, run t12.
